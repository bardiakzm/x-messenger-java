...
public class UserPage extends JPanel {
    static private boolean nextTweetSaveProtected = false;
    static private boolean nextTweetFollowingProtected = false;
    ...
    public UserPage(String username, List<Tweet> followingTweets, List<Tweet> randomTweets, StartPage parentFrame) {
        ...
        JButton saveProtectedButton = new JButton("not save protected");
        JButton followingProtectedButton = new JButton("all");

        // Create a new panel with FlowLayout for the buttons
        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        buttonPanel.add(followingProtectedButton);
        buttonPanel.add(saveProtectedButton);
        ...
        saveProtectedButton.addActionListener(e -> toggleSaveProtected(saveProtectedButton));
        followingProtectedButton.addActionListener(e -> toggleFollowingProtected(followingProtectedButton));
    ...
    private void showTweets(String tweetType, JButton parentButton, StartPage parentFrame) {
        ...
        for (Tweet tweeti : tweets) {
            ...
            if (!tweet.followingProtected || tweetPublisher.followers.contains(username) || tweetPublisher.username.equals(username)) {
                ...
                if(!tweet.publisherid.equals(username)) {
    ...
    private void composeTweet(String text, JButton parentButton) {
        Packet.sendtweet(username, text,nextTweetSaveProtected,nextTweetFollowingProtected);
        updateTweetLists();
    ...
    static void toggleSaveProtected(JButton saveProtectedButton){
        if(nextTweetSaveProtected){
            nextTweetSaveProtected = false;
            saveProtectedButton.setText("not save protected");
        }else{
            nextTweetSaveProtected = true;
            saveProtectedButton.setText("save protected");
        }
    }

    static void toggleFollowingProtected(JButton followingProtectedButton){
        if(nextTweetFollowingProtected){
            nextTweetFollowingProtected = false;
            followingProtectedButton.setText("all");
        }else{
            nextTweetFollowingProtected = true;
            followingProtectedButton.setText("followers only");
        }
    }
}
